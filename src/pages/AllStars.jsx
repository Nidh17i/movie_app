...
export const TopMovies = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#0f0f0f] text-white px-10 py-12">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-extrabold mb-10 tracking-wide border-b border-gray-700 pb-4">
          Top Rated Movies
        </h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8">
          {topMovies.map((movie) => (
            <Link key={movie.id} to={`/movie/${movie.id}`}>
              <div
                key={movie.id}
                className="relative bg-[#1f1f1f] rounded-2xl overflow-hidden shadow-lg hover:scale-[1.03] hover:shadow-xl transition-all duration-300"
              >
                <img
                  src={
                    movie.poster_path
